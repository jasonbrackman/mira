...
class TaskItem(object):
    def __init__(self, asset_type_sequence, asset_name_shot, step, task, status, status_color, start_date, due_date, pix_map):
...
class MyTaskForm(QWidget):
    ...
    def set_model(self):
        ...
        for task in my_tasks:
            entity_type = self.__db.get_task_entity_type(task)
            ...
            if entity_type == "Asset":
                asset_type_sequence = self.__db.get_asset_type_by_asset_id(task_entity_id)
            else:
                asset_type_sequence = self.__db.get_sequence_by_shot_id(task_entity_id)
            ...
            scaled = pix_map.scaled(150, 100, Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
            task_item = TaskItem(asset_type_sequence, task_entity_name, step, task_name,
    ...
    def filter_finish(self, state):
        if state:
            ...
            self.proxy_model.set_name_filter("^((?!Delivered).)*$")
        self.close_delegate()
        self.set_delegate()
