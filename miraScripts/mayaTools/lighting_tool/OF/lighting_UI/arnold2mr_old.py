...
of_mat_dict = {'aiOf_aiLayerMixer':  {'Color0': {'mr_name': 'inputs[0].color', 'value': (1.0, 1.0, 1.0)},
                                      'Mask0': {'mr_name': 'inputs[0].alpha', 'value': 1.0},
                                      'Color1': {'mr_name': 'inputs[1].color', 'value': (0.0, 0.0, 0.0)},
                                      'Mask1': {'mr_name': 'inputs[1].alpha', 'value': 1.0},
                                      'Color2': {'mr_name': 'inputs[2].color', 'value': (0.0, 0.0, 0.0)},
                                      'Mask2': {'mr_name': 'inputs[2].alpha', 'value': 1.0},
...
def create_mr_material(shader_type, shader_name):
    if not pm.objExists(shader_name):
        if shader_type == 'aiStandard':
            ...
            shader = pm.shadingNode('layeredTexture', asTexture=1, name=shader_name)
        return shader
    else:
...
def connect_forward_attr(ar_shader, mr_shader):
    ...
    for node in destination_nodes:
        ...
        elif node.type() == 'layeredShader':
            surface_shader = create_surface_shader()
            if mr_shader.type() == 'mia_material_x':
                mr_shader.result >> surface_shader.outColor
            elif mr_shader.type() == 'misss_fast_skin_maya':
                mr_shader.outValue >> surface_shader.outColor
            dst_attrs = ar_shader.outputs(type='layeredShader', plugs=1)
            for attr in dst_attrs:
                src_attr = attr.inputs(plugs=1)[0]
                src_attr // attr
                new_src_attr = re.sub(ar_shader.name(), surface_shader.name(), src_attr.name())
                pm.PyNode(new_src_attr) >> attr
        elif node.type() != 'aiOf_aiLayerMixer':
...
def convert_ailayermixer(material):
    mat_type = material.type()
    ar_name = material.name()
    texture_name = ar_name + '_texture'
    new_name = ar_name + '_mr'
    layered_texture_node = create_mr_material(mat_type, texture_name)
    mr_material = create_mr_material('aiStandard', new_name)
    connect_forward_attr(material, mr_material)
    layered_texture_node.outColor >> mr_material.diffuse
    layered_texture_node.outAlpha >> mr_material.diffuseA
    ...
    for attr in all_attr:
        ...
        if source_attr[0]:
            if source_attr[1].node().type() == 'aiStandard':
                ...
                surface_shader = create_surface_shader()
                mr_material_new.result >> surface_shader.outColor
                #mr_src_attr = surfaceShader.outColor
                mr_src_attr = re.sub(source_attr[1].node().name(), surface_shader.name(), source_attr[1].name())
                #mr_dst_attr = layeredTexture.inputs[1].color
                mr_dst_attr = re.sub(ar_name, layered_texture_node.name(), '%s.%s' % (ar_name, attr))
                mr_dst_attr = re.sub(attr, of_mat_dict[mat_type][attr]['mr_name'], mr_dst_attr)
                pm.PyNode(mr_src_attr) >> pm.PyNode(mr_dst_attr)
                ...
                if not 'Mask' in attr:
                    ...
                    reverse_shader.outputX >> mr_dst_attr
