...
class CheckPhong(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        layout_of_frame = QVBoxLayout(frame)
        ...
        separate_frame = QFrame()
        separate_frame.setStyleSheet('QFrame{color: #118811}')
        separate_frame.setFrameStyle(QFrame.HLine)
        separate_layout.addWidget(separate_frame)
        #---------------------------------list widget------------------------------------------------#
        self.phong_list = QListWidget()
        self.phong_list.setStyleSheet("QListWidget{background-color: transparent}")
        self.phong_list.setSelectionMode(QListWidget.ExtendedSelection)
        self.phong_list.setSortingEnabled(True)
        #---------------------------------Convert    ------------------------------------------------#
        self.convert_btn = QPushButton('Convert All Phong Shader To aiStandard')
    ...
    def set_background(self):
        ...
        palette = QPalette()
        palette.setBrush(QPalette.Background, QBrush(self.image.scaled(self.size(),
                         Qt.IgnoreAspectRatio, Qt.SmoothTransformation)))
        self.setPalette(palette)

    def resizeEvent(self, event):
        palette = QPalette()
        palette.setBrush(QPalette.Background, QBrush(self.image.scaled(event.size(),
                         Qt.IgnoreAspectRatio, Qt.SmoothTransformation)))
        self.setPalette(palette)
    ...
    def init_settings(self):
        ...
        if phong_list:
            ...
            self.close()
            QMessageBox.information(None, "Information", "No Phong")
...
class PhongItem(QListWidgetItem):
