...
class TaskManager(task_manager_ui.TaskManagerUI):
    ...
    def show_path(self):
        ...
        if entity_type == "Asset":
            template = pipeMira.get_site_value(self.__project, "maya_asset_work")
            file_path = template.format(primary=self.__primary, project=self.__project, asset_type=first_selected[0],
                                        ...
                                        task=fourth_selected[0], version="000", engine="maya")
        else:
            template = pipeMira.get_site_value(self.__project, "maya_shot_work")
    ...
    def create_origin_file(self, file_name):
        ...
        start_dir = join_path.join_path2(scripts_dir, "pipeTools", "start")
