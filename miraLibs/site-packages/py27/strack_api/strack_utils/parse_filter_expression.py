...
def parse_filter_expression(expression_str):
    # 元组转列表
    expression_str = tuple_to_list(expression_str)
    # 切分并解析
...
def exp_to_dict(exp_str):
    ...
    if len(exp_list) == 3:
        ...
        value_str = reformat_time_list(exp_list[2].strip())
        # list_str to list
        if value_str and (value_str[0], value_str[-1]) == ("[", "]"):
...
def reformat_time_list(value_str):
...
if __name__ == "__main__":
    ...
    ex = "id in [1,2,3,4,5]"    # -> "id in 1,2,3,4,5"
    # ex = "first_name== cheng"
