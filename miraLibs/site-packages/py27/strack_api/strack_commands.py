...
class CreateCommand(Command):
    ...
    def _init_data(self, payload):
        ...
        if status_info and (status_info.get("name") not in fields_dict):
            status = status_info.get("value") if status_info.get(
                "value") != "default" else self.server.status.find().get("id")
            fields_dict.update({status_info.get("name"): status})  # set default status
        data_dict = {
...
class PopCommand(Command):
    ...
    def __init__(self, server_object, cmd, params, entity=None):
        super(PopCommand, self).__init__(server_object, cmd, params, entity)
    ...
    def _init_requests(self, args, kwargs):
        ...
        if payload.get("entity_type"):
            data_dict.update({"entity": payload.pop("entity_type")})
        if payload.get("count"):
            data_dict.update({"number": payload.pop("count")})
        else:
            data_dict.update({"number": 1})
        payload["data"] = json.dumps(data_dict)
        return payload, None

    def _format_result(self, result):
        for event_info in result:
            type_id = event_info.pop("type")
            event_info[u"entity_type"] = self.EVENT_ENTITY_TYPE_MAP.get(str(type_id))
            event_info[u"new_value"] = event_info.pop("new_val")
            event_info[u"old_value"] = event_info.pop("old_val")

            field = event_info.get("field")
            old_value = event_info.get("old_value") or {}
            new_value = event_info.get("new_value") or {}

            if event_info.get("mode") == "changed":
                entity = event_info.get("entity_type")
                old_value_dict = self._map_int_to_str([event_info.get("entity_type"), {field: old_value}])
                old_value = old_value_dict.get(field)
                new_value_dict = self._map_int_to_str([event_info.get("entity_type"), {field: new_value}])
                new_value = new_value_dict.get(field)
                event_info[u"old_value"] = old_value
                event_info[u"new_value"] = new_value

            elif event_info.get("mode") in ["created", "remove"]:
                if event_info.get("mode") == "created":
                    entity = event_info.get("entity_type")
                    if isinstance(new_value, dict):
                        new_value["type"] = entity
                        new_value_dict = self._map_fields([entity, new_value])
                        new_value = self._map_int_to_str([entity, new_value_dict])
                        event_info[u"new_value"] = new_value
                elif event_info.get("mode") == "remove":
                    if isinstance(old_value, dict):
                        entity = self.ENTITY_TYPE_MAP.get(old_value.get("type"))
                        old_value["type"] = entity
                        old_value_dict = self._map_fields([entity, old_value])
                        old_value = self._map_int_to_str([entity, old_value_dict])
                        event_info[u"old_value"] = old_value
