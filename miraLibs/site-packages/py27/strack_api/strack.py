...
class Strack(object):
    ...
    def __init__(self, base_url, login, api_key):
        ...
        # init get web url command
        self.__get_web_url = Command(self, "console/site", None)
    ...
    @property
    def web_url(self):
        url_info = self.__get_web_url()
        if url_info:
            return url_info.get("system")
    ...
    @property
    def entities(self):
        return sorted(self._entities_detail().keys())
    ...
    def get_sign_info(self):
        ...
        if response.json()["status"] == 200:
            ...
            self.__sign_code = sign_info.get("token", "")
            self.__preference = sign_info.get("preference", {})
    ...
    def get_unique_code():
        ...
        # generate the code
        node = uuid.getnode()
        unique_code = uuid.UUID(int=node).hex[-12:]
        # save cache
...
class Entity(object):
    ...
    def help(self):
        ...
        """
        help_info = ["Help on Strack Entity type '%s' object" % self.name]
        # find description from doc folder
        doc_path = os.path.join(current_dir, "docs", "%s.json" % self.name)
        description = ""
        if os.path.isfile(doc_path):
            with open(doc_path) as f:
                doc_dict = json.loads(f.read())
                description = doc_dict.get("entry", "")
        if not description:
            description = self.__server.general_help.get("entry", "")
        description = description % self.name
        description += "\n    get all methods by: `st.%s.methods`" % self.name
        description += "\n    get all fields by: `st.%s.fields`" % self.name
        description += "\n"
        help_info.append(description)
        # list static fields
        static_field_list = VALUE_MAP.get(self.name)
        if static_field_list:
            help_info.append("Static Fields:")
            help_info.append("\t %-20s %-20s %-20s" % ("field_name", "supported options", "database value"))
            help_info.append("-" * 64)
            for field, value_list in static_field_list.iteritems():
                help_info.append("\t %-20s %-20s %-20s" % (field, "", ""))
                for value_pair in value_list:
                    help_info.append("\t %-20s %-20s %-20s" % ("", value_pair[1], value_pair[0]))
        else:
            help_info.append("No Static Fields")
        return "\n".join(help_info)
    ...
    def creation_field(self):
        source_fields = self.__detail.get("createFields")
        if source_fields:
            user_fields = [self.__server.map_field(self.name, i) for i in source_fields]
            return sorted(user_fields)

    @property
    def update_field(self):
        source_fields = self.__detail.get("updateFields")
        if source_fields:
            user_fields = [self.__server.map_field(self.name, i) for i in source_fields]
            return sorted(user_fields)
    ...
    def default_field(self):
        source_fields = self.__detail.get("defaultFields")
        user_fields = [self.__server.map_field(self.name, i) for i in source_fields]
        return sorted(user_fields)
    ...
    @property
    def has_custom(self):
        return self.__detail.get("hasCustom")
    ...
    def relations(self):
        ...
        if relation_fields_dict:
            return sorted(relation_fields_dict.keys())
        return []

    @property
    def default_fields(self):
        return sorted(self.__detail.get("defaultFields"))

    @property
    def methods(self):
        return sorted(self.__methods.keys())


if __name__ == "__main__":
    st = Strack(base_url="http://192.168.120.65/strack_api/public",
                login="aaron", api_key="ad26bebc-21cd-425e-8d2e-c05f969713f3")
    # print st.entities
    # help(st.user.find)
