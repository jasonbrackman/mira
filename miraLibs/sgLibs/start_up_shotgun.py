...
import aas_sg
import aas_os
import OddOrange.oskit as oskit



logging.basicConfig(filename=os.path.join(os.environ["TMP"], 'aas_libs_start_up_shotgun_log.txt'),
...
def start_up_shotgun(project_name="df"):
    ...
    # get tk
    tk = aas_sg.get_tk_object()
    # get path
    os_type = oskit.get_os_type()
    if os_type == "osx":
        os_type = "mac"
    ...
    # get current user
    sg_login = aas_os.get_shotgun_login()
